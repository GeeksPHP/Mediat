...
final class Webfairy
{
            ...
            if ((($rows = $this->cache->get($cache_id)) == false) || $fresh == true) {
                ...
                foreach ($data->where($conditions) as $value) {
                    ...
                    $rows[$value['id']] = array(
                        ...
                        'animated' => ((isset($attrs['animated-gif']) && (boolean) $attrs['animated-gif'] == true) || ($type == 2 && $value['provider'] == 'giphy')) ,
    ...
    public function db_post_array($data = array(), $update = false, $post_id = 0)
    {
        ...
        if (isset($data['file']) == true) {
            ...
            if ($result['type'] == 1) {
                if($data['file']['file_mime_type'] == 'image/gif'){
                    $file_path = $this->_uploaded('path', $data['file']['file_physical_name']);
                    if(self::isAni($file_path)){
                        $result['attrs']['animated-gif'] = 1;
                    }
                }
    ...
    public function urlinfo_array($url, $process = true)
    {
        ...
        $records = array(
            ...
            'link' => (isset($data['url'])) ? $data['url'] : $url,
            'link' => (isset($data['url'])) ? $data['url'] : $url,
            'title' => '',
            'description' => '',
            'content' => '',
        ...
        if (isset($thumb_key) == true && $records['thumb_id'] == 0) {
            if ($thumb_arr = $this->grabImage($data[$thumb_key], false, true, array('title' => $records['title']), $process,array(),'jpg')) {
    ...
    public function grabImage($url, $local = false, $db_save = false, $attrs = array(), $process = true,$loadurl_options = array(),$force_convert = '')
    {
        if ($process == false) {
            return $url;
        }
        ...
        if(empty($force_convert) == false){
            switch($extension){
                case 'jpg':
                case 'jpeg':
                    $imageTmp = imagecreatefromjpeg($tmp_file_path);
                    break;
                case 'png':
                    $imageTmp = imagecreatefrompng($tmp_file_path);
                    break;
                case 'gif':
                    $imageTmp = imagecreatefromgif($tmp_file_path);
                    break;
                case 'bmp':
                    $imageTmp = imagecreatefrombmp($tmp_file_path);
                    break;
            }
            switch($force_convert){
                case 'jpg':
                    if(!imagejpeg($imageTmp, $tmp_file_path, 80)){
                        return false;
                    }
                    $mime_type = 'image/jpg';
                    break;
                case 'png':
                    if(!imagepng($imageTmp, $tmp_file_path, 9)){
                        return false;
                    }
                    $mime_type = 'image/png';
                    break;
            }
            $extension = $force_convert;
    ...
    public static function isAni($filename) {
