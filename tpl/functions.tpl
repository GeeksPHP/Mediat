{strip}
{function resized_src filename='' s=''}{if Webfairy::is_absolute_url($filename)}{$filename}{else}{$config.base_url}/resized/{$s}_{$filename}{/if}{/function}
{function resized filename='' s='small' title='' attrs=[] responsive=true original=''}<img {if $responsive}class="img-responsive"{/if} {if $original == 'self'}data-original="{resized_src filename=$filename s=$s}"{elseif empty($original) == false}data-original="{$original}"{/if} src="{resized_src filename=$filename s=$s}" alt="{$title}" title="{$title}" {foreach $attrs as $attr_key => $attr_value}{$attr_key}="{$attr_value}" {/foreach}/>{/function}
        ...
        <li class="post-item post-id-{$post.id} post-type-{$post.type} no-js">
            ...
            <div class="item-large">
                ...
                <div class="post-body">
                    ...
                    {elseif empty($post.thumb) == false}
                        <div class="post-image {$post.provider} {if $post.animated}animated-gif{/if}">
                            {if $post.animated}
                                {if $post.provider == 'giphy'}
                                    {resized filename=$post.thumb responsive=false original='self' s='auto-height-medium' title=$post.title attrs=['class' => 'animation','data-animation' => $post.link,'data-state' => 0]}
                                    {else}
                                    {resized filename=$post.thumb responsive=false original='self' s='auto-height-medium' title=$post.title attrs=['class' => 'animation','data-animation' => "{$config.base_url}/uploads/{$post.file.file_physical_name}",'data-state' => 0]}
                                {/if}
                                <p class="gif-play">
