<?php
    ...
    if($post['animated']){
        if($post['type'] == 1){
            $og_link_image = $Webfairy->image($post['file']['file_physical_name'], '', false);
        }elseif($post['provider'] == 'giphy'){
            $og_link_image = $post['link'];
        }
        $property_meta['og:image'] = $property_meta['og:url'] = $og_link_image;
    }
